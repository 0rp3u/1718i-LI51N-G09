...
class TMDBService2 : MoviesDataSource {
    companion object {
        private val gson: Gson = Gson()
        val TAG = TMDBService2::class.java.simpleName
    ...
    override fun popularMovies(page: Int) : List<Movie>{
         return get(Uri.Builder().appendEncodedPath("movie/popular")).let {
    ...
    override fun upcomingMovies(page: Int) : List<Movie>{
        return get(Uri.Builder().appendEncodedPath("movie/upcoming")).let {
            DataMapper().mapToMovieList(gson.fromJson(it.toString(), MovieSearchResult::class.java))
        }
    }

    override fun playingMovies (page: Int): List<Movie> {
        return get(Uri.Builder().appendEncodedPath("movie/now_playing")).let {
            DataMapper().mapToMovieList(gson.fromJson(it.toString(), MovieSearchResult::class.java))
        }
    }
    ...
    override fun movieSearch(query: String, page: Int) : List<Movie> {
        return get(
                Uri.Builder()
                    ...
                    .appendQueryParameter("query", query)).let {
            DataMapper().mapToMovieList(gson.fromJson(it.toString(), MovieSearchResult::class.java))
        }
    }
